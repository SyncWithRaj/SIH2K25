...
// --- Tab Content Components (No changes needed here) ---
                ...
                {['graduation', 'twelfth', 'tenth'].map(level => (
                    <div key={level} className="p-6 bg-gray-50 border border-gray-200 rounded-xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <input name="board" value={data.educationDetails?.[level]?.board || ''} onChange={(e) => handleNestedChange(level, e)} placeholder="Board" className="p-3 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
...
export default function ProfilePage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Use a more modern notification if possible, but alert is fine for now
                ...
                <nav className="space-y-4 w-full">
                    {TABS.map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)} className={`w-full text-left py-3 px-5 rounded-lg font-semibold text-md transition-all duration-200 flex items-center ${activeTab === tab ? 'bg-indigo-600 text-white shadow-lg' : 'hover:bg-gray-200 text-gray-700'}`}>
                            {/* You can add icons here later if you like */}
                    ...
                    <div className="bg-white p-6 sm:p-8 lg:p-12 rounded-2xl shadow-lg">
                        ...
                        <div className="flex justify-end mt-10 pt-6 border-t border-gray-200">
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                                className="px-10 py-3 bg-indigo-600 text-white font-bold rounded-lg shadow-md disabled:bg-indigo-300 disabled:cursor-not-allowed hover:bg-indigo-700 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                            >
